...
class PrivatePilotAdditionalInfo extends Component{
    ...
    practicalChangeHandler = (event) => {
        if( this.state.practicalSelected !== event.target.name){
            ...
            this.props.handleChange({target: {name: "A33_practical", value: event.target.name}});
        }
    }


    render(){
        if(this.props.endorsements.includes("A33")) {
            return (
                <>
                    <Row className="mb-4">
                        <h2 className="float-left">Private Pilot Endorsement - Additional Information</h2>
                    </Row>
                    <Row>
                        <Form className="w-50">
                            <Form.Group as={Row}>
                                <Form.Label column md="5">Private Pilot Practical test: </Form.Label>
                                <Col md="7">
...
export default PrivatePilotAdditionalInfo;